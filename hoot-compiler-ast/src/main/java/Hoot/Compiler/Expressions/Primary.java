...
public class Primary extends Operand {
    ...
    public boolean hasItem() { return hasOne(this.item); }
    ...
    @Override public boolean containsExit() { return hasItem() && this.item.exitsMethod(); }
    @Override public boolean producesPredicate() { return hasItem() && this.item.producesPredicate(); }
    @Override public boolean hasOnlyValue() { return !isExpression() && !isBlock(); }
    @Override public boolean selfIsPrimary() { return hasItem() && this.item.selfIsPrimary(); }
    ...
    @Override public Emission emitPrimitive() { return hasItem()? this.item.emitPrimitive(): emitEmpty(); }
    @Override public Emission emitOperand()   { return hasItem()? this.item.emitOperand(): emitEmpty(); }
